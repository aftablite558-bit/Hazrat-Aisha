...
export function ReportCard() {
  ...
  return (
    <div className="bg-[var(--bg-surface-raised)] min-h-screen pb-12 print:bg-white print:pb-0 font-body">
      <div className="max-w-4xl mx-auto pt-6 px-4 print:pt-0 print:px-0">
        
        {/* Controls - Hidden in Print */}
        <div className="flex justify-between mb-6 print:hidden">
          <Button variant="secondary" onClick={() => navigate(-1)} className="border-line text-content-secondary hover:text-content font-bold font-display bg-surface shadow-e1 hover:bg-surface-raised">
            <ArrowLeft className="h-4 w-4 mr-2" /> Back
          </Button>
          <Button onClick={() => window.print()} className="font-bold font-display shadow-e1">
            <Printer className="h-4 w-4 mr-2" /> Print Report Card
          </Button>
        </div>

        {/* Report Card Page */}
        <div className="bg-surface border border-line shadow-e2 rounded-2xl print:border-none print:shadow-none print:w-full print:max-w-full mx-auto print:rounded-none transition-all duration-fast font-display" style={{ width: '210mm', minHeight: '297mm', padding: '20mm' }}>
          ...
          <div className="flex justify-between items-start mb-8">
            ...
            {student.photo ? (
              <img src={student.photo} alt="Student" className="w-24 h-32 object-cover border-2 border-line rounded-lg shadow-sm ml-4" referrerPolicy="no-referrer" />
            ) : (
              <div className="w-24 h-32 border-2 border-dashed border-line rounded-lg flex flex-col items-center justify-center text-content-tertiary text-xs text-center ml-4 bg-surface-raised font-bold">
            ...
            <table className="w-full border-collapse text-sm min-w-[500px]">
              <thead className="bg-surface-overlay border-b border-line">
                <tr>
                  ...
                  <th className="border-r border-line p-3 text-left font-bold text-content-secondary text-xs uppercase tracking-wider">Subject</th>
                  <th className="border-r border-line p-3 text-center w-24 font-bold text-content-secondary text-xs uppercase tracking-wider">Max Marks</th>
                  <th className="border-r border-line p-3 text-center w-24 font-bold text-content-secondary text-xs uppercase tracking-wider">Pass Marks</th>
              ...
              <tbody className="divide-y divide-line">
                {exam.subjects.map((sub, index) => {
                  ...
                  return (
                    <tr key={sub.id} className="bg-surface hover:bg-surface-raised transition-colors duration-fast">
                      <td className="border-r border-line p-3 text-center font-mono font-bold text-content-secondary">{index + 1}</td>
                      <td className="border-r border-line p-3 font-bold text-content">{sub.name}</td>
                      <td className="border-r border-line p-3 text-center font-mono font-semibold text-content-secondary">{sub.maxMarks}</td>
                      <td className="border-r border-line p-3 text-center font-mono font-semibold text-content-secondary">{sub.passingMarks}</td>
                ...
                <tr className="bg-surface-overlay font-black border-t border-line text-content">
                  <td colSpan={2} className="border-r border-line p-4 text-right uppercase tracking-wider text-xs">Grand Total</td>
                  <td className="border-r border-line p-4 text-center font-mono">{exam.subjects.reduce((sum, s) => sum + s.maxMarks, 0)}</td>
                  <td className="border-r border-line p-4 text-center"></td>
          ...
          {/* Summary Box */}
          <div className="grid grid-cols-2 sm:grid-cols-4 border border-line rounded-xl overflow-hidden bg-surface-raised divide-x divide-y sm:divide-y-0 divide-line font-display mb-12 shadow-sm">
          ...
          <div className="flex justify-between items-end pt-12">
            <div className="text-center w-48 font-display">
              <div className="border-b border-line mb-2 h-12"></div>
              <p className="text-xs font-bold text-content-tertiary uppercase tracking-wider">Class Teacher</p>
            </div>
            
            <div className="text-center flex flex-col items-center">
              <div className="p-2 bg-white rounded-xl border border-line shadow-sm">
            ...
            <div className="text-center w-48 font-display">
              <div className="border-b border-line mb-2 h-12 flex items-end justify-center pb-1">
